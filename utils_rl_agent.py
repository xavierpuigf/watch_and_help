...
class GraphHelper():
    def __init__(self):
        ...
        self.object_dict = DictObjId(self.objects + ['no_obj'])
    ...
    def build_graph(self, graph, ids):
        ...
        edge_ids = np.array([[id2index[edge['from_id']], id2index[edge['to_id']]] for edge in edges])

        mask_edges = np.zeros(max_edges)
        all_edge_ids = np.zeros((max_edges, 2))
        all_edge_types = np.zeros((max_edges))

        mask_nodes = np.zeros((max_edges))
        ...
        return all_class_names, node_states, all_edge_ids, all_edge_types, mask_nodes, mask_edges

...
def write_video(log_file, out_file, comm, file_folder, file_folder_log):
    ...
    s, im = comm.camera_image([cam1], image_height=480, image_width=640)
    s, im2 = comm.camera_image([cam1], mode='seg_class', image_height=480, image_width=640)
    s, info_cam = comm.camera_data([81])
    ...
    new_action_2 = []
    cont = 0
    for it_act in range(len(action_1)):
        current_action = action_1[it_act]
        if action_1[it_act] is not None and 'walk' in action_1[it_act]:
            if it_act < len(action_1) - 1 and action_1[it_act] == action_1[it_act+1]:
                cont += 1
                new_action_1.append(None)
            else:
                new_action_1.append('{} :{}:'.format(current_action, cont + 1))
                cont = 0

        else:
            new_action_1.append(current_action)
            cont = 0
    cont = 0
    for it_act in range(len(action_2)):
        current_action = action_2[it_act]
        if action_2[it_act] is not None and 'walk' in action_2[it_act]:
            if it_act < len(action_2) - 1 and action_2[it_act] == action_2[it_act+1]:
                new_action_2.append(None)
                cont += 1
            else:
                new_action_2.append('{} :{}:'.format(current_action, cont + 1))
                cont = 0

        else:
            new_action_2.append(current_action)
            cont = 0

    action_1 = new_action_1
    action_2 = new_action_2
    pdb.set_trace()

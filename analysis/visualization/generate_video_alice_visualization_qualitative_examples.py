...
def write_video(log_file, out_file, comm, file_folder, file_folder_log):
    ...
    graphs = [g]
    # pdb.set_trace()
    print([i for i in range(98) if 385 in [node['id'] for node in content['obs'][i * 2]]])
    # ipdb.set_trace()
    ...
    for it, action in enumerate(tqdm(action_1)):
        ...
        s, m = comm.render_script([action_str],
                           recording=False, gen_vid=False, camera_mode=["96"], #["PERSON_FROM_BACK"],
                           image_synthesis=['normal'], frame_rate=5, output_folder=file_folder,
                           image_width=512, image_height=384, save_pose_data=False,
                           smooth_walk=smooth_walk, file_name_prefix=out_file, processing_time_limit=250, time_scale=ts)
        # ipdb.set_trace()
        # s, m = comm.render_script([action_str],
...
if __name__ == '__main__':
    ...
    print("START")
    if args.use_editor:
        out_folder = '/Users/xavierpuig/Desktop/test_videos_qualitative_examples/'
        file_names = [
            ...
            '/Users/xavierpuig/Downloads/logs_agent_64_setup_table_read_book_1.pik'
        ...
        if args.use_docker:
            ...
            with open('../../record_scratch/rec_good_test/info_progrs_SWS.json', 'r') as f:
                programs_SWS = json.load(f)
                programs_SWS = sorted(programs_SWS, key=lambda x: x[1])
            programs_selected = programs_SWS[-30:]
            file_names = [x[0] for x in programs_selected] + ['multiAlice_env_task_set_20_check_neurips_test/' + x[0].split('/')[-1] for x
                                              in programs_selected]

        file_name_files = home_path + '/split/all_watch.txt'


    # pdb.set_trace()
    for itf, file_name_part in enumerate(tqdm(file_names)):
        file_name = file_name_part
        splitf = file_name.split('/')[-2] +'_' + file_name.split('/')[-1].split('.')[0]
        file_video_gen = '{}/vids_gen_qualitative/{}.json'.format(home_path, splitf)

        if False: #os.path.isfile(file_video_gen):
            continue
        else:

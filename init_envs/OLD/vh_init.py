...
class SetInitialGoal:
    ...
    def check_goal_achievable(self, graph, comm, env_goal, apartment):
        ...
        if (self.task_name == 'setup_table') or (self.task_name == 'put_dishwasher') or (
            ...
            for goal in env_goal[self.task_name]:
                ...
                self.object_id_count, graph = self.add_obj(graph_copy, obj, num_obj, self.object_id_count,
    ...
    def add_obj(self, graph, obj_name, num_obj, object_id, objs_in_room=None, only_position=None, except_position=None,
                goal_obj=False):
    ...
    def setup_table(self, graph, start=True):
        ...
        for k, v in self.goal.items():
            ...
            self.object_id_count, graph = self.add_obj(graph, k, num_obj, self.object_id_count,
                                                       objs_in_room=objs_in_room, except_position=except_position_ids,
                                                       goal_obj=True)

        if start:
            self.object_id_count, graph = self.setup_other_objs(graph, self.object_id_count, objs_in_room=objs_in_room,
    ...
    def clean_table(self, graph, start=True):
        ...
        for k, v in self.goal.items():
            ...
            num_obj = random.randint(v, self.init_pool[k]['env_max_num'] + 1)  # random select objects >= goal
            self.object_id_count, graph = self.add_obj(graph, k, v, self.object_id_count, objs_in_room=objs_in_room,
                                                       only_position=table_id,
                                                       goal_obj=True)  ## add the first v objects on this table
            self.object_id_count, graph = self.add_obj(graph, k, num_obj - v, self.object_id_count,
...
parser.add_argument('--display', type=str, default='0', help='Task name')
...
parser.add_argument('--exec_file', action='store_true', default=True, help='Use unity editor')

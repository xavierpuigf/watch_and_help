...
class UnityEnv:
    def __init__(self, 
        ...
        self.env_task_set_files = glob.glob('{}/*.pik'.format(env_task_set_file))
        ...
        if self.simulator_type == 'unity':
            ...
            self.observation_space = spaces.Dict({
                ...
                'planner_action': spaces.Box(low=0, high=num_actions, shape=(1,1)),
                'planner_object': spaces.Box(low=0, high=self.graph_helper.num_objects, shape=(1,1))
            })
        ...
        self.spec = envs.registration.EnvSpec('virtualhomebc-v0')
    ...
    def in_room(self, center, room_nodes):


        for room_node in room_nodes:
            bbox = room_node['bounding_box']
            x1 = bbox['center'][0] - bbox['size'][0]/2.0
            x2 = bbox['center'][0] + bbox['size'][0]/2.0
            z1 = bbox['center'][2] - bbox['size'][2]/2.0
            z2 = bbox['center'][2] + bbox['size'][2]/2.0
            if center[0] > x1 and center[0] < x2 and center[2] > z1 and center[2] < z2:
                return room_node['id']
        pdb.set_trace()


    def update_graph_seq(self, unity_graph, task_seq, actions):
        new_graph_seq = []
        current_action_list = zip(actions[0], actions[1])
        room_nodes = [node for node in unity_graph['nodes'] if node['category'] == 'Rooms']

        room_nodes_ids = [node['id'] for node in room_nodes]
        did_walk_to_room = [False, False]
        objects_moved = []
        for action_str in actions[0]+actions[1]:
            if action_str is None:
                continue
            act, obj = parse_script_line(action_str)
            if act.lower() == 'grab':
                objects_moved.append(obj[0][1])

        edges_objects_not_moved = [edge for edge in unity_graph['edges'] if edge['from_id'] not in objects_moved and edge['to_id'] not in objects_moved]
        room_obj = {edge['from_id']: edge['to_id'] for edge in edges_objects_not_moved if edge['relation_type'] == 'INSIDE' and edge['to_id'] in room_nodes_ids}
        for it, nodes_step in enumerate(task_seq):
            if actions[1][it] is None:
                break
            new_graph_seq.append({
                'nodes': nodes_step,
                'edges': edges_objects_not_moved
            })

        try:
            new_graph_seq[0]['edges'] = [edge for edge in unity_graph['edges'] if edge['from_id'] not in [1,2]]
        except:
            pdb.set_trace()
        # Start putting the objects in place
        current_edges = [edge for edge in new_graph_seq[0]['edges']]
        for it, (ac1, ac2) in enumerate(current_action_list):
            # set up the previous edge
            if ac2 is None:
                break
            new_graph_seq[it]['edges'] = [edge for edge in current_edges]

            if ac1 is not None:
                ac1, obj1 = parse_script_line(ac1)
                if ac1.lower() == 'grab':
                    object_grabbed = obj1[0][1]
                    current_edges = [edge for edge in current_edges if not (edge['from_id'] == object_grabbed or edge['to_id'] == object_grabbed)]
                    current_edges += [{'from_id': 1, 'to_id': object_grabbed, 'relation_type': 'HOLDS_RH'}]
                if 'put' in ac1.lower():
                    object_placed = obj1[1][1]
                    object_grabbed = obj1[0][1]
                    current_edges = [edge for edge in current_edges if not ('HOLDS' in edge['relation_type'] and edge['from_id'] == 1)]
                    if 'putin' in ac1.lower():
                        rel_type = 'INSIDE'
                    else:
                        rel_type = 'ON'
                        current_edges.append(
                        {'from_id': object_grabbed, 'to_id': room_obj[object_placed], 'relation_type': 'INSIDE'})
                    current_edges.append({'from_id': object_grabbed, 'to_id': object_placed, 'relation_type': rel_type})

            if ac2 is not None:
                ac2, obj2 = parse_script_line(ac2)
                if ac2.lower() == 'grab':
                    object_grabbed = obj2[0][1]
                    current_edges = [edge for edge in current_edges if
                                     not (edge['from_id'] == object_grabbed or edge['to_id'] == object_grabbed)]
                    current_edges += [{'from_id': 2, 'to_id': object_grabbed, 'relation_type': 'HOLDS_RH'}]
                if 'put' in ac2.lower():
                    object_placed = obj2[1][1]
                    object_grabbed = obj2[0][1]
                    current_edges = [edge for edge in current_edges if
                                     not ('HOLDS' in edge['relation_type'] and edge['from_id'] == 2)]
                    if 'putin' in ac2.lower():
                        rel_type = 'INSIDE'
                    else:
                        rel_type = 'ON'
                        current_edges.append(
        ...
        for it, graph in enumerate(new_graph_seq):
            ...
            if obj2[0][1] not in room_nodes_ids:
                r2 = [edge['to_id'] for edge in graph['edges'] if edge['from_id'] == obj2[-1][1] and edge['to_id'] in room_nodes_ids][0]
            else:
                charp2 = [node['bounding_box']['center'] for node in graph['nodes'] if node['id'] == 2][0]
                r2 = self.in_room(charp2, room_nodes)
        ...
        for it, gr in enumerate(new_graph_seq):
            ...
            if len(set(objects_moved+[1,2]) - set(objects_in_graph)) > 0:
                if it == (len(new_graph_seq) - 1):
                    if 1 not in objects_in_graph:
                        gr['edges'].append({'from_id': 1, 'relation_type': 'INSIDE', 'to_id': room_nodes_ids[0]})
                    if 2 not in objects_in_graph:
                        gr['edges'].append({'from_id': 2, 'relation_type': 'INSIDE', 'to_id': room_nodes_ids[0]})
                    pdb.set_trace()
            if len([edge for edge in gr['edges'] if edge['from_id'] in [1,2] and edge['relation_type'] == 'ON']) > 0:
                pdb.set_trace()
        return new_graph_seq
    ...
    def reset(self, graph=None, task_goal=None):
        ...
        self.task_goal = {0: env_task['goals'], 1: env_task['goals']}
        ...
        curr_graph_system_agent = self.inside_not_trans(self.init_graph)
        self.env.reset(curr_graph_system_agent, self.task_goal)
        self.env.to_pomdp()
        obs = self.get_observations()[0]


        self.prev_dist = None # self.get_distance()

        self.num_steps = 0
        return obs

    def reset_2agents_python(self):
        env_task = random.choice(self.env_task_set_files)
    ...
    def get_action_from_command(self, action_str):

        action, params = parse_script_line(action_str)
        action = action.lower().replace('walk', 'walktowards')
        ...
        if 'put' in action.lower():
            ...
            o1 = params[0][1]
        action_id = self.graph_helper.action_dict.get_id(action)
        return action_id, o1
    ...
    def get_observations(self, mode='seg_class', image_width=None, image_height=None, drawing=False):
        if self.simulator_type == 'unity':
            ...
            rel_coords = torch.Tensor(list([0, 0]))[None, :]
            curr_graph = self.env_task['curr_graph'][self.step_count]

            if self.observation_type == 'mcts':
                # Use the mcts function to get the obs
                #python_graph = {'nodes': self.env_task['curr_graph'][self.step], 'edges': []}
                inside_graph = self.inside_not_trans(curr_graph)

                python_graph = self.env.get_observations(char_index=1, graph_env=inside_graph)
            ...
            graph_inputs, graph_viz = self.graph_helper.build_graph(python_graph, ids=visible_objects,
                                                                    character_id=self.my_agent_id, plot_graph=drawing,
                                                                    level=self.level)
            self.visible_nodes = graph_viz[-1]
            self.observed_graph = curr_graph
            ...
            action_str = self.env_task['action'][1][self.step_count]
            action_interact, object_interact_id = self.get_action_from_command(action_str)
            object_interact = np.where(object_interact_id == current_obs['node_ids']) # TODO: match here id with
            if len(object_interact[0]) == 0:
            ...
            current_obs.update(
                {
                    ...
                    'planner_action': np.array([action_interact])[:, None],
                    'planner_object': object_interact[0][:, None]
                }
            )



            return current_obs, (images[0], curr_graph, graph_viz)

...
class Random_agent:
    ...
    def get_action(self, obs, goal_spec, opponent_subgoal=None):
        ...
        if action_name == 'walk':
            ...
            objects = [(node['class_name'], node['id']) for node in obs['nodes'] if
                       node['class_name'] in content['objects_grab']]
        elif action_name == 'put':
            objects = [(node['class_name'], node['id']) for node in obs['nodes'] if
                       node['class_name'] in content['objects_surface'] + content['objects_inside']]
        else:
            objects = [(node['class_name'], node['id']) for node in obs['nodes'] if
                       node['class_name'] in content['objects_inside']]

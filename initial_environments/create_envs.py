...
class EnvironmentCreator:
    ...
    def get_surface_ids(self, environment):
        return [node['id'] for node in environment['nodes'] if 'SURFACE' in node['properties']]
    ...
    def create_objects(example_node, num_objects):
        new_nodes = []
    ...
    def build_relations(nodes_place, nodes_inside, nodes_ontop, room_ids, constraints=None):
        edges = []
    ...
    def transform_environment(self, environment):
        ...
        for object_type_id in range(len(objects_place)):
            ...
            if len(objects_place[object_type_id]) == 0:
                ...
                ipdb.set_trace()
                raise Exception
            node_objects = create_objects(example_node, num_objects_to_place)
            all_nodes += node_objects

            all_edges += build_relations(node_objects, nodes_inside, nodes_ontop, room_ids)

        
        nodes_place = [place_nodes(objects_place[i][0])]

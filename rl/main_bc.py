...
def main():
    ...
    envs = make_vec_envs(env_info, args.num_steps, simulator_type, args.seed, args.num_processes,
    ...
    for j in range(num_updates):
        ...
        for step in range(args.num_steps):
            ...
            with torch.no_grad():
                value, action, action_log_prob, recurrent_hidden_states = actor_critic.act(
                    ...
                    rollouts.masks[step], epsilon=epsilon, action_indices=[rollouts.obs['planner_action'][step], rollouts.obs['planner_object'][step]])
            ...
            print(step, reward[0].item(), done[0].item(), 'action loss', -action_log_prob[0].mean().item(),
                  'object loss', -action_log_prob[1].mean().item())
            total_num_steps += 1
        ...
        value_loss, action_loss, dist_entropy = agent.update(rollouts, ce=True)

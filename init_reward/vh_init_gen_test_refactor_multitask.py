...
if __name__ == "__main__":
    ...
    file_split = '/data/vision/torralba/frames/data_acquisition/SyntheticStories/MultiAgent/challenge/data_challenge/split/watch_scenes_mutliple_split_names.json'
    ...
    for elem in content_split['test']:
        ...
        if pred_str not in cache_files:
            ...
            pdb.set_trace()
            continue
        predicates['test'].append((elem['pred_dict'], elem['task_name_comb'], pred_str))
    ...
    for predicates_dict, task_name_combo, pred_str in predicates['test']: # test
        ...
        #print('env_id:', env_id)
        print('task_name:', task_name_combo)
        ...
        for i in range(num_test):
            ...
            apt_list = [capt for capt in apartment_list if not any([task_name not in task_names[capt+1] for task_name in task_name_combo])]
            ...
            print('------------------------------------------------------------------------------')
            print('testing %d: %s' % (i, task_name_combo))
            print('------------------------------------------------------------------------------')
            ...
            task_name = task_name_combo[0] + '_' + task_name_combo[1]
